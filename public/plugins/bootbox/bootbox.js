...
(function (root, factory) {
  ...
  let defaults = {
    ...
    locale: 'en',
    // Show backdrop or not. Default to static so customer has to interact with dialog
    backdrop: 'static',
  ...
  exports.dialog = function (options) {
    ...
    if (options.backdrop === true) {
      ...
      // A boolean true/false according to the Bootstrap docs should show a dialog the customer can dismiss by clicking on the background.
    ...
    dialog.on('click', '.bootbox-close-button', function (e) {
      // onEscape might be falsy, but that's fine; the fact is if the customer has managed to click the close button we have to close the dialog, callback or not
      processCallback(e, dialog, callbacks.onEscape);
    });
  ...
  exports.alert = function () {
    ...
    // Override the ok and escape callback to make sure they just invoke the single customer-supplied one (if provided)
  ...
  exports.confirm = function () {
    ...
    // Overrides; undo anything the customer tried to set they shouldn't have
  ...
  exports.prompt = function () {
    ...
    // Capture the customer's 'show' value; we always set this to false before spawning the dialog to give us a chance to attach some handlers to it, but we need to make sure we respect a preference not to show it
    ...
    if ($.trim(options.message) !== '') {
      // Add the form to whatever content the customer may have added.
  ...
  // Merge a set of default dialog options with customer supplied arguments
  ...
  function mergeDialogOptions(className, labels, properties, args) {
    ...
    // Ensure the buttons properties generated, *after* merging with customer args are still valid against the supplied labels
    return validateButtons(
      // Merge the generated base properties with customer supplied arguments
      mergeArguments(
        baseOptions,
        args,
  ...
  // Filter and tidy up any customer supplied parameters to this dialog.
